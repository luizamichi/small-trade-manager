...
function delete(): bool {
	...
	elseif(\controller\service\null($values) || empty($values['id'])) // INFORMOU PARÂMETROS INVÁLIDOS NA REQUISIÇÃO
		...
		$query .= 'delete from service_budget where service=' . $id . ';';
		...
		$query .= 'delete from service_sale where service=' . $id . ';';
		...
		if($operation) { // O SERVIÇO FOI REMOVIDO DA BASE DE DADOS
			...
			\mysql\log($reference=$id, $action='delete', $entity='services');
...
function insert(): bool {
	...
	elseif(\controller\service\null($values)) // INFORMOU PARÂMETROS INVÁLIDOS NA REQUISIÇÃO
		...
		else // ECONTROU ERROS NOS VALORES E NÃO INSERIU O SERVIÇO
			\api\util\response(
				$status=406,
				$success=false,
