...
function delete(): bool {
	...
	elseif(\controller\product\null($values) || empty($values['id'])) // INFORMOU PARÂMETROS INVÁLIDOS NA REQUISIÇÃO
		...
		$query .= 'delete from product_budget where product=' . $id . ';';
		...
		$query .= 'delete from product_sale where product=' . $id . ';';
		...
		if($operation) { // O PRODUTO FOI REMOVIDO DA BASE DE DADOS
			...
			\mysql\log($reference=$id, $action='delete', $entity='products');
...
function insert(): bool {
	...
	elseif(\controller\product\null($values)) // INFORMOU PARÂMETROS INVÁLIDOS NA REQUISIÇÃO
		...
		else // ECONTROU ERROS NOS VALORES E NÃO INSERIU O PRODUTO
			\api\util\response(
				$status=406,
				$success=false,
