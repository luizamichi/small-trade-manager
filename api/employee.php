...
function delete(): bool {
	...
	elseif(\controller\employee\null($values) || empty($values['id'])) // INFORMOU PARÂMETROS INVÁLIDOS NA REQUISIÇÃO
		...
		foreach($tuples as $tuple) {
			$query .= 'delete from product_budget where sale=' . $tuple->id . ';';
			$query .= 'delete from service_budget where sale=' . $tuple->id . ';';
		}
		...
		foreach($tuples as $tuple)
			$query .= 'delete from product_pruchase where sale=' . $tuple->id . ';';
		...
		if($operation) { // O FUNCIONÁRIO FOI REMOVIDO DA BASE DE DADOS
			...
			\mysql\log($reference=$id, $action='delete', $entity='employees');
...
function insert(): bool {
	...
	elseif(\controller\employee\null($values)) // INFORMOU PARÂMETROS INVÁLIDOS NA REQUISIÇÃO
		...
		if(!\controller\employee\errors($tuple)) { // NENHUM ERRO ENCONTRADO NOS VALORES
			...
			$operation = \mysql\execute($query, $tuple->permission);
		...
		else // ECONTROU ERROS NOS VALORES E NÃO INSERIU O FUNCIONÁRIO
			\api\util\response(
				$status=406,
				$success=false,
