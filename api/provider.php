...
function delete(): bool {
	...
	elseif(\controller\provider\null($values) || empty($values['id'])) // INFORMOU PARÂMETROS INVÁLIDOS NA REQUISIÇÃO
		...
		foreach($products as $tuple)
			$query .= 'delete from product_budget where product=' . $tuple->id . ';';
		if(count($products) > 0)
			$tuples = \mysql\execute('select * from product_budget where product in(' . array_map($products, function($tuple) { return $tuple->id; }) . ');');
		foreach($tuples as $tuple)
			$query .= 'delete from budgets where id=' . $tuple->budget . ';';

		// REMOVE TODAS AS VENDAS COM PRODUTOS DO FORNECEDOR
		foreach($products as $tuple)
			$query .= 'delete from product_sale where product=' . $tuple->id . ';';
		if(count($products) > 0)
			$tuples = \mysql\execute('select * from product_sale where product in(' . array_map($products, function($tuple) { return $tuple->id; }) . ');');
		foreach($tuples as $tuple)
		...
		if($operation) { // O FORNECEDOR FOI REMOVIDO DA BASE DE DADOS
			// SALVA O REGISTRO DA REMOÇÃO
			\mysql\log($reference=$id, $action='delete', $entity='providers');
...
function insert(): bool {
	...
	elseif(\controller\provider\null($values)) // INFORMOU PARÂMETROS INVÁLIDOS NA REQUISIÇÃO
		...
		else // ECONTROU ERROS NOS VALORES E NÃO INSERIU O FORNECEDOR
			\api\util\response(
				$status=406,
				$success=false,
