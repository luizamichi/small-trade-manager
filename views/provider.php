...
function index(): bool {
	if(!\controller\session\authenticate('provider')) {
		...
		require_once(__DIR__ . '/../templates/provider/index.php'); // CARREGA O TEMPLATE DE FORNECEDORES (INDEX)
		return true;
	}
}
...
function insert(): bool {
	if(!\controller\session\authenticate('provider')) {
		...
		require_once(__DIR__ . '/../templates/provider/insert.php'); // CARREGA O TEMPLATE DE FORNECEDORES (INSERT)
		return true;
	}
}
...
function update(): bool {
	if(!\controller\session\authenticate('provider'))
		...
		if(isset($operation[0])) {
			...
			require_once(__DIR__ . '/../templates/provider/update.php'); // CARREGA O TEMPLATE DE FORNECEDORES (UPDATE)
			return true;
		}
...
function view(): bool {
	if(!\controller\session\authenticate('provider'))
		...
		if(isset($operation[0])) {
			...
			require_once(__DIR__ . '/../templates/provider/view.php'); // CARREGA O TEMPLATE DE FORNECEDORES (VIEW)
			return true;
		}
