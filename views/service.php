...
function index(): bool {
	if(!\controller\session\authenticate('service')) {
		...
		require_once __DIR__ . '/../templates/service/index.php'; // CARREGA O TEMPLATE DE SERVIÇOS (INDEX)
		return true;
	}
}
...
function insert(): bool {
	if(!\controller\session\authenticate('service')) {
		...
		require_once __DIR__ . '/../templates/service/insert.php'; // CARREGA O TEMPLATE DE SERVIÇOS (INSERT)
		return true;
	}
}
...
function update(): bool {
	if(!\controller\session\authenticate('service')) {
		...
		if(isset($operation[0])) {
			...
			require_once __DIR__ . '/../templates/service/update.php'; // CARREGA O TEMPLATE DE SERVIÇOS (UPDATE)
			return true;
		}
...
function view(): bool {
	if(!\controller\session\authenticate('service')) {
		...
		if(isset($operation[0])) {
			...
			require_once __DIR__ . '/../templates/service/view.php'; // CARREGA O TEMPLATE DE SERVIÇOS (VIEW)
			return true;
		}
