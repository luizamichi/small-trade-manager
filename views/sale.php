...
function index(): bool {
	if(!\controller\session\authenticate('sale')) {
		...
		require_once(__DIR__ . '/../templates/sale/index.php'); // CARREGA O TEMPLATE DE VENDAS (INDEX)
		return true;
	}
}
...
function insert(): bool {
	if(!\controller\session\authenticate('sale')) {
		...
		require_once(__DIR__ . '/../templates/sale/insert.php'); // CARREGA O TEMPLATE DE VENDAS (INSERT)
		return true;
	}
}
...
function update(): bool {
	if(!\controller\session\authenticate('sale'))
		...
		if(isset($operation[0])) {
			...
			$products = \mysql\execute($query);
			$tuple->products = [];
			foreach($products as $product_sale) {
				$query = 'select * from products where id=' . $product_sale->product . ';';
				$product = \mysql\execute($query)[0];

				$product->quantity = $product_sale->quantity;
				$product->unit_price = $product_sale->unit_price;
				$product->total_price = $product_sale->unit_price * $product_sale->quantity;
				array_push($tuple->products, $product);
			}
			...
			$services = \mysql\execute($query);
			$tuple->services = [];
			foreach($services as $service_sale) {
				$query = 'select * from services where id=' . $service_sale->service . ';';
				$service = \mysql\execute($query)[0];

				$service->quantity = $service_sale->quantity;
				$service->unit_price = $service_sale->unit_price;
				$service->total_price = $service_sale->unit_price * $service_sale->quantity;
				array_push($tuple->services, $service);
			}
			...
			require_once(__DIR__ . '/../templates/sale/update.php'); // CARREGA O TEMPLATE DE VENDAS (UPDATE)
			return true;
		}
...
function view(): bool {
	if(!\controller\session\authenticate('sale'))
		...
		if(isset($operation[0])) {
			...
			require_once(__DIR__ . '/../templates/sale/view.php'); // CARREGA O TEMPLATE DE VENDAS (VIEW)
			return true;
		}
