...
function index(): bool {
	if(!\controller\session\authenticate('product')) {
		...
		require_once(__DIR__ . '/../templates/product/index.php'); // CARREGA O TEMPLATE DE PRODUTOS (INDEX)
		return true;
	}
}
...
function insert(): bool {
	if(!\controller\session\authenticate('product')) {
		...
		require_once(__DIR__ . '/../templates/product/insert.php'); // CARREGA O TEMPLATE DE PRODUTOS (INSERT)
		return true;
	}
}
...
function update(): bool {
	if(!\controller\session\authenticate('product'))
		...
		if(isset($operation[0])) {
			...
			require_once(__DIR__ . '/../templates/product/update.php'); // CARREGA O TEMPLATE DE PRODUTOS (UPDATE)
			return true;
		}
...
function view(): bool {
	if(!\controller\session\authenticate('product'))
		...
		if(isset($operation[0])) {
			...
			require_once(__DIR__ . '/../templates/product/view.php'); // CARREGA O TEMPLATE DE PRODUTOS (VIEW)
			return true;
		}
