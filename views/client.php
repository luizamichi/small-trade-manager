...
function index(): bool {
	if(!\controller\session\authenticate('client')) {
		...
		require_once(__DIR__ . '/../templates/client/index.php'); // CARREGA O TEMPLATE DE CLIENTES (INDEX)
		return true;
	}
}
...
function insert(): bool {
	if(!\controller\session\authenticate('client')) {
		...
		require_once(__DIR__ . '/../templates/client/insert.php'); // CARREGA O TEMPLATE DE CLIENTES (INSERT)
		return true;
	}
}
...
function update(): bool {
	if(!\controller\session\authenticate('client'))
		...
		if(isset($operation[0])) {
			...
			require_once(__DIR__ . '/../templates/client/update.php'); // CARREGA O TEMPLATE DE CLIENTES (UPDATE)
			return true;
		}
...
function view(): bool {
	if(!\controller\session\authenticate('client'))
		...
		if(isset($operation[0])) {
			...
			require_once(__DIR__ . '/../templates/client/view.php'); // CARREGA O TEMPLATE DE CLIENTES (VIEW)
			return true;
		}
