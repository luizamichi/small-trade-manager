	...
	<div class="container mb-5">
		...
		<form accept-charset="utf-8" action="<?=BASE_URL . 'action/' . PAGE_NAME?>/select/" autocomplete="on" class="form-inline justify-content-center" enctype="application/x-www-form-urlencoded" id="search-provider" method="get" name="search-provider" rel="noopener" target="_self">
			<div class="form-row align-items-center">
				<div class="col-auto">
					...
					<div class="input-group mb-2">
						...
						<input class="form-control" form="search-provider" id="search" minlength="1" name="all" placeholder="Procurar" required type="search"/>
					</div>
				</div>
				<div class="col-auto">
		...
		<div class="table-responsive">
			<table class="table table-hover table-sm">
				...
				<thead class="thead-light">
					<tr>
						<th id="company_name" scope="row">Razão Social</th>
						<th id="fantasy_name" scope="row">Nome Fantasia</th>
						<th id="cnpj" scope="row">CNPJ</th>
						<th id="city" scope="row">Cidade</th>
						<th id="state" scope="row">Estado</th>
				...
				<tbody>
					...
					<tr class="text-truncate" id="<?=$tuple->id?>">
						<td><?=$tuple->company_name?></td>
						<td><?=$tuple->fantasy_name?></td>
						<td><?=$tuple->cnpj?></td>
						<td><?=$tuple->city?></td>
						<td><?=$tuple->state?></td>
						<td>
