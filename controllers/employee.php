...
function duplicate(object $tuple): array {
	...
	$query = 'select * from employees where email = "' . $tuple->email . '"' . ($tuple->id > 0 ? ' && id != ' . $tuple->id : '') . ';';
	$operation = \mysql\execute($query);
	if($operation) { // ENCONTROU UM E-MAIL VINCULADO A OUTRO FUNCIONÁRIO
		array_push($problems, 'e-mail pertence a outro funcionário');
	}

	$query = 'select * from employees where cell_phone = "' . $tuple->cell_phone . '"' . ($tuple->id > 0 ? ' && id != ' . $tuple->id : '') . ';';
	$operation = \mysql\execute($query);
	if($operation) { // ENCONTROU UM CELULAR VINCULADO A OUTRO FUNCIONÁRIO
		array_push($problems, 'celular pertence a outro funcionário');
	}
...
function filter(string $method='GET'): array {
	...
	$array['alias'] = filter_input($method, 'alias', FILTER_DEFAULT);
	$array['password'] = filter_input($method, 'password', FILTER_DEFAULT);
