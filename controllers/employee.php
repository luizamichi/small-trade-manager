...
function beautify(object $tuple): object {
	...
	$tuple->sex = $tuple->sex == 'M' ? 'Masculino' : ('F' ? 'Feminino' : 'Indefinido');
	return $tuple;
}
...
function convert(array $values): object {
	...
	$array['permission'] = (object) $array['permission'];
	return (object) $array;
}


/** VALIDA SE O FUNCIONÁRIO POSSUI ALGUM ATRIBUTO ÚNICO VINCULADO A OUTRO FUNCIONÁRIO
 * @param object $tuple
 * @return array
 */
function duplicate(object $tuple): array {
	...
	$query = 'select * from employees where alias="' . $tuple->alias . '"' . ($tuple->id > 0 ? ' && id!=' . $tuple->id : '') . ';';
	$operation = \mysql\execute($query);
	if($operation) // ENCONTROU UM APELIDO VINCULADO A OUTRO FUNCIONÁRIO
		array_push($problems, 'apelido pertence a outro funcionário');

	$query = 'select * from employees where rg="' . $tuple->rg . '"' . ($tuple->id > 0 ? ' && id!=' . $tuple->id : '') . ';';
	$operation = \mysql\execute($query);
	if($operation) // ENCONTROU UM RG VINCULADO A OUTRO FUNCIONÁRIO
		array_push($problems, 'RG pertence a outro funcionário');

	$query = 'select * from employees where cpf="' . $tuple->cpf . '"' . ($tuple->id > 0 ? ' && id!=' . $tuple->id : '') . ';';
	$operation = \mysql\execute($query);
	if($operation) // ENCONTROU UM CPF VINCULADO A OUTRO FUNCIONÁRIO
		array_push($problems, 'CPF pertence a outro funcionário');
...
function filter(string $method='GET'): array {
	...
	$array['surname'] = filter_input($method, 'surname', FILTER_SANITIZE_STRING);

	$array['alias'] = filter_input($method, 'alias', FILTER_SANITIZE_STRING);
	$array['password'] = filter_input($method, 'password', FILTER_SANITIZE_STRING);

	$array['rg'] = str_replace('.', '', str_replace('-', '', filter_input($method, 'rg', FILTER_SANITIZE_NUMBER_INT)));
	$array['rg'] = is_numeric($array['rg']) ? $array['rg'] : null;

	$array['cpf'] = str_replace('.', '', str_replace('-', '', filter_input($method, 'cpf', FILTER_SANITIZE_NUMBER_INT)));
	$array['cpf'] = is_numeric($array['cpf']) ? $array['cpf'] : null;

	$array['birthday'] = filter_input($method, 'birthday', FILTER_SANITIZE_NUMBER_INT);
	$array['postal_code'] = str_replace('-', '', filter_input($method, 'postal-code', FILTER_SANITIZE_NUMBER_INT));
	$array['district'] = filter_input($method, 'district', FILTER_SANITIZE_STRING);
	$array['city'] = filter_input($method, 'city', FILTER_SANITIZE_STRING);
	$array['state'] = filter_input($method, 'state', FILTER_SANITIZE_STRING);
	$array['address'] = filter_input($method, 'address', FILTER_SANITIZE_STRING);
	...
	$array['complement'] = filter_input($method, 'complement', FILTER_SANITIZE_STRING);
	$array['email'] = filter_input($method, 'email', FILTER_SANITIZE_STRING, FILTER_SANITIZE_MAGIC_QUOTES);

	$array['phone'] = str_replace('(', '', str_replace(')', '', str_replace('-', '', filter_input($method, 'phone', FILTER_SANITIZE_NUMBER_INT))));
	$array['phone'] = is_numeric($array['phone']) ? $array['phone'] : null;

	$array['cell_phone'] = str_replace('(', '', str_replace(')', '', str_replace('-', '', filter_input($method, 'cell-phone', FILTER_SANITIZE_NUMBER_INT))));
	$array['cell_phone'] = is_numeric($array['cell_phone']) ? $array['cell_phone'] : null;

	$array['sex'] = filter_input($method, 'sex', FILTER_SANITIZE_STRING, FILTER_SANITIZE_MAGIC_QUOTES);
	$array['note'] = filter_input($method, 'note', FILTER_SANITIZE_STRING);

	$array['permission'] = filter_input($method, 'permission', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY);
	$array['permission'] = is_array($array['permission']) ? $array['permission'] : [];

	foreach($array as $index => $value) { // CONVERTE TODOS OS VALORES EM STRING
		if(is_array($value)) {
			$values = ['budget', 'client', 'employee', 'product', 'provider', 'purchase', 'record', 'report', 'sale', 'setting', 'service'];
			$permissions = [];

			foreach($values as $permission)
				$permissions[$permission] = (int) in_array($permission, $value);
			$array[$index] = $permissions;
		}
		else
			$array[$index] = trim($value) ?: null;
	}
	return $array;
}
...
function null(array $values): bool {
	...
	foreach($values as $index => $value)
		if((is_array($values[$index]) && empty($values[$index])) || $values[$index] != null) {
			$null++;
		}
	return !$null;
}
...
function rectify(array $values): array {
	...
	$array['permission'] = $values['permission'] ?? [];
	return $array;
}


/** VALIDA SE O FUNCIONÁRIO ESTÁ ATENDENDO TODOS OS REQUISITOS EXIGIDOS E RETORNA UMA LISTA COM OS PROBLEMAS
 * @param object $tuple
 * @return array
 */
function validate(object $tuple): array {
	...
	if(strlen($tuple->alias) < 6 || strlen($tuple->alias) > 32 || strpos($tuple->alias, ' ')) // INFORMOU UM APELIDO COM TAMANHO INVÁLIDO
		array_push($problems, 'apelido');

	if(strlen($tuple->password) < 6) // INFORMOU UMA SENHA COM TAMANHO INVÁLIDO
		array_push($problems, 'senha');

	if(strlen($tuple->rg) != 0 && (!in_array(strlen($tuple->rg), [8, 9]) || !is_numeric($tuple->rg))) // INFORMOU UM RG COM TAMANHO INVÁLIDO
	...
	if(!checkdate(substr($tuple->birthday, 5, 2), substr($tuple->birthday, 8, 2), substr($tuple->birthday, 0, 4)) || $tuple->birthday > date('Y-m-d')) // INFORMOU UMA DATA DE NASCIMENTO INVÁLIDA
		array_push($problems, 'data de nascimento');

	if(strlen($tuple->postal_code) != 0 && (strlen($tuple->postal_code) != 8 || !is_numeric($tuple->postal_code))) // INFORMOU UM CEP COM TAMANHO INVÁLIDO
		array_push($problems, 'CEP');

	if(strlen($tuple->district) != 0 && (strlen($tuple->district) < 4 || strlen($tuple->district) > 32)) // INFORMOU UM BAIRRO COM TAMANHO INVÁLIDO
	...
	if(strlen($tuple->complement) != 0 && (strlen($tuple->complement) < 4 || strlen($tuple->complement) > 32)) // INFORMOU UM COMPLEMENTO COM TAMANHO INVÁLIDO
	...
	if(strlen($tuple->phone) != 0 && (strlen($tuple->phone) != 10 || !is_numeric($tuple->phone))) // INFORMOU UM TELEFONE INVÁLIDO
		array_push($problems, 'telefone');

	if(strlen($tuple->cell_phone) != 0 && (strlen($tuple->cell_phone) != 11 || !is_numeric($tuple->cell_phone))) // INFORMOU UM CELULAR INVÁLIDO
		array_push($problems, 'celular');

	if(!in_array($tuple->sex, ['F', 'M'])) // INFORMOU UM SEXO INVÁLIDO
		array_push($problems, 'sexo');

	if(strlen($tuple->note) != 0 && (strlen($tuple->note) < 4 || strlen($tuple->note) > 512)) // INFORMOU UMA OBSERVAÇÃO COM TAMANHO INVÁLIDO
		array_push($problems, 'observação');

	if(count((array) $tuple->permission) == 0) // NÃO INFORMOU NENHUMA PERMISSÃO
		array_push($problems, 'permissão');
