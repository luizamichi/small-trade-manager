...
function beautify(object $tuple): object {
	...
	$tuple->entity = ['budgets' => 'Orçamentos', 'clients' => 'Clientes', 'employees' => 'Funcionários', 'products' => 'Produtos', 'providers' => 'Fornecedores', 'purchases' => 'Compras', 'sales' => 'Vendas', 'settings' => 'Configurações', 'services' => 'Serviços'][$tuple->entity];
	...
	$tuple->day = substr($tuple->day, 8, 2) . '/' . substr($tuple->day, 5, 2) . '/' . substr($tuple->day, 0, 4) . ' - ' . \substr($tuple->day, 10);
	return $tuple;
}
...
function filter(string $method='GET'): array {
	...
	$array['action'] = filter_input($method, 'action', FILTER_SANITIZE_STRING);
	$array['entity'] = filter_input($method, 'entity', FILTER_SANITIZE_STRING);
	...
	$array['description'] = filter_input($method, 'description', FILTER_SANITIZE_STRING);
