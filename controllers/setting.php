...
function filter(string $method='GET'): array {
	...
	$array['website'] = filter_input($method, 'website', FILTER_SANITIZE_STRING, FILTER_SANITIZE_URL);
...
function validate(object $tuple): array {
	...
	if(strlen($tuple->website) != 0 && (strlen($tuple->website) < 8 || strlen($tuple->website) > 32 || !filter_var($tuple->website, FILTER_VALIDATE_URL))) // INFORMOU UM E-MAIL INVÁLIDO
