...
function filter(string $method='GET'): array {
	...
	$array['company_name'] = filter_input($method, 'company-name', FILTER_SANITIZE_STRING);
	$array['fantasy_name'] = filter_input($method, 'fantasy-name', FILTER_SANITIZE_STRING);

	$array['state_registration'] = str_replace('.', '', filter_input($method, 'state-registration', FILTER_SANITIZE_NUMBER_INT));
	$array['state_registration'] = is_numeric($array['state_registration']) ? $array['state_registration'] : null;

	$array['cnpj'] = str_replace('.', '', str_replace('-', '', str_replace('/', '', filter_input($method, 'cnpj', FILTER_SANITIZE_NUMBER_INT))));
...
function validate(object $tuple): array {
	...
	if(strlen($tuple->state_registration) != 0 && (strlen($tuple->state_registration) != 12 || !is_numeric($tuple->state_registration))) // INFORMOU UMA INSCRIÇÃO ESTADUAL COM TAMANHO INVÁLIDO
	...
	if(!checkdate(substr($tuple->foundation_date, 5, 2), substr($tuple->foundation_date, 8, 2), substr($tuple->foundation_date, 0, 4))) // INFORMOU UMA DATA DE FUNDAÇÃO INVÁLIDA
