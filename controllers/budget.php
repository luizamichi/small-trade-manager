...
/** VALIDA SE O ORÇAMENTO POSSUI ALGUM ATRIBUTO ÚNICO VINCULADO A OUTRA ORÇAMENTO
 * @param object $tuple
 * @return array
 */
...
function reduce(array $array): array {
	...
	for($i = 0; $i < $count; $i++) {
		if(in_array($array['products'][$i], $products)) {
			...
			array_push($quantities, $array['products_quantities'][$i]);
			array_push($prices, $array['products_prices'][$i]);
		}
	}
	...
	for($i = 0; $i < $count; $i++) {
		if(in_array($array['services'][$i], $services)) {
			...
			array_push($services, $array['services'][$i]);
			array_push($quantities, $array['services_quantities'][$i]);
			array_push($prices, $array['services_prices'][$i]);
		}
	}
	$array['services'] = $services;
	$array['quantities'] = $quantities;
	$array['prices'] = $prices;
	$array['cart'][] = array_map(function($service, $quantity, $price) {
...
/** VALIDA SE O ORÇAMENTO ESTÁ ATENDENDO TODOS OS REQUISITOS EXIGIDOS E RETORNA UMA LISTA COM OS PROBLEMAS
 * @param object $tuple
 * @return array
 */
function validate(object $tuple): array {
	...
	if(!is_array($tuple->products_quantities) || count($tuple->products_quantities) <= 0 && (is_array($tuple->products) && count($tuple->products) > 0)) // INFORMOU UMA QUANTIDADE DE PRODUTOS INVÁLIDA
		array_push($problems, 'quantidades dos produtos');

	if((!is_array($tuple->services_quantities) || count($tuple->services_quantities) <= 0) && (is_array($tuple->services) && count($tuple->services) > 0)) // INFORMOU UMA QUANTIDADE DE SERVIÇOS INVÁLIDA
		array_push($problems, 'quantidades dos serviços');

	if(!is_array($tuple->products_prices) || count($tuple->products_prices) <= 0 && (is_array($tuple->products) && count($tuple->products) > 0)) // INFORMOU UM PREÇO DE PRODUTO INVÁLIDO
		array_push($problems, 'preços dos produtos');

	if(!is_array($tuple->services_prices) || count($tuple->services_prices) <= 0 && (is_array($tuple->services) && count($tuple->services) > 0)) // INFORMOU UM PREÇO DE SERVIÇO INVÁLIDO
