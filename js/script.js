$(document).ready(function() {
	...
	$("input[name='cell-phone']").focusout(function() {
		if($(this).val().replace(/[^0-9]/gim, "").length == 11)
			$(this).addClass("is-valid").removeClass("is-invalid");
	...
	$("input[name='cpf']").focusout(function() {
		let cpf = $(this).val().replace(/[^0-9]/gim, "");
		let erro = false;
		if(cpf.length != 11)
			erro = true;
	...
	$("input[name='cnpj']").focusout(function() {
		let cnpj = $(this).val().replace(/[^0-9]/gim, "");
		let erro = false;
		if(cnpj.length != 14)
			erro = true;
	...
	$("select[name='form-of-payment'], select[name='sex'], select[name='situation'], select[name='source'], select[name='state'], select[name='unity']").click(function() {
		if($(this).children("option:selected").val() || $(this).children("optgroup").children("option:selected").val())
			$(this).addClass("is-valid").removeClass("is-invalid");
	});
	...
	$("input[name='phone']").focusout(function() {
		if($(this).val().replace(/[^0-9]/gim, "").length == 10)
			$(this).addClass("is-valid").removeClass("is-invalid");
		else if($(this).val().length == 0 && !$(this).prop("required"))
			$(this).removeClass("is-invalid is-valid");
		else
	...
	$("input[name='postal-code']").focusout(function() {
		let status;
		$.ajax({
			url: "https://viacep.com.br/ws/" + $(this).val().replace(/[^0-9]/, "") + "/json/",
			dataType: "jsonp",
			crossDomain: true,
	...
	$("input[name='rg']").focusout(function() {
		if($(this).val().replace(/[^0-9]/gim, "").length == 8 || $(this).val().replace(/[^0-9]/gim, "").length == 9)
			$(this).addClass("is-valid").removeClass("is-invalid");
		else if($(this).val().length == 0 && !$(this).prop("required"))
			$(this).removeClass("is-invalid is-valid");
		else
	...
	$("input[name='state-registration']").focusout(function() {
		if($(this).val().replace(/[^0-9]/gim, "").length == 12)
			$(this).addClass("is-valid").removeClass("is-invalid");
		else if($(this).val().length == 0 && !$(this).prop("required"))
			$(this).removeClass("is-invalid is-valid");
		else
	...
	$("input[name='workload']").focusout(function() {
		if($(this).val().trim().length == 5 && /^([0-1]?[0-9]|2[0-4]):([0-5][0-9])(:[0-5][0-9])?$/.test($(this).val().trim()))
			$(this).addClass("is-valid").removeClass("is-invalid");
		else
			$(this).addClass("is-invalid").removeClass("is-valid");
	});
	...
	$("input[name='alias']").mask("ABBBBB##########################", {translation: {
		"#": {
			pattern: /[a-z0-9]/,
			optional: true
		},
	...
	$("button[id='add-product']").click(function() {
		if(parseInt($("input[id='product-quantity']").val().trim()) >= 1 && $("datalist[id='products'] option").map(function() { return $(this).val(); }).get().includes($("input[id='product']").val().trim()) && !$("input[name='products[]']").map(function() { return $(this).val(); }).get().includes($("input[id='product']").val().trim())) {
			...
			let remove = "<div class='col-md-1 form-group'><br/><button class='btn btn-danger mt-2' data-remove='remove-product' type='button'>Remover</button></div>";
	...
	$("button[id='add-service']").click(function() {
		if(parseInt($("input[id='service-quantity']").val().trim()) >= 1 && $("datalist[id='services'] option").map(function() { return $(this).val(); }).get().includes($("input[id='service']").val().trim()) && !$("input[name='services[]']").map(function() { return $(this).val(); }).get().includes($("input[id='service']").val().trim())) {
			...
			let remove = "<div class='col-md-1 form-group'><br/><button class='btn btn-danger mt-2' data-remove='remove-service' type='button'>Remover</button></div>";
	...
	$("form[method='post']").submit(function() {
		...
		$("select").each(function() { // VALIDAÇÃO DE TODOS OS CAMPOS "SELECT"
			if($(this).prop("required") && !$(this).children("option:selected").val()) {
				$(this).addClass("is-invalid");
				error = true;
			}
		});
	...
	$("form[method='get']").submit(function() {
		$.ajax({
			...
			error: function() {
				$("#flash-message").text("Não foi possível efetuar o cadastro, houve um erro de comunicação com o servidor.");
				$("#flash-message").addClass("alert-danger").removeClass("alert-success alert-primary alert-warning");
			}
		});
		return false;
